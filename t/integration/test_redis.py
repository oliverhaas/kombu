...
def get_connection(hostname, port, vhost, user_name=None, password=None, transport_options=None):
    credentials = f"{user_name}:{password}@" if user_name else ""

    return kombu.Connection(f"redis://{credentials}{hostname}:{port}", transport_options=transport_options)
...
def test_RedisConnectTimeout(monkeypatch):
    ...
    monkeypatch.setattr(redis.connection.Connection, "_connect", connect_timeout)
...
def test_RedisConnection_check_hostname(monkeypatch):
    ...
    def connect_check_certificate(self):
        ...
        raise TimeoutError("check_hostname=False")

    monkeypatch.setattr(redis.connection.SSLConnection, "_connect", connect_check_certificate)
