...
class BasicFunctionality:
    ...
    def test_publish_consume(self, connection):
        ...
        with connection as conn, conn.channel() as channel:
    ...
    def test_consume_empty_queue(self, connection):
        ...
        with connection as conn, conn.channel():
            consumer = kombu.Consumer(conn, [test_queue], accept=["pickle"])
            consumer.register_callback(callback)
            with consumer, pytest.raises(socket.timeout):
                conn.drain_events(timeout=1)

    def test_simple_queue_publish_consume(self, connection):
        with connection as conn, closing(conn.SimpleQueue("simple_queue_test")) as queue:
    ...
    def test_simple_buffer_publish_consume(self, connection):
        with connection as conn, closing(conn.SimpleBuffer("simple_buffer_test")) as buf:
...
class BaseExchangeTypes:
    ...
    def _create_consumer(self, connection, queue):
        consumer = kombu.Consumer(connection, [queue], accept=["pickle"])
    ...
    def _publish(self, channel, exchange, queues=None, routing_key=None):
        ...
        if routing_key:
            ...
            producer.publish({"hello": "world"}, declare=list(queues) if queues else None, serializer="pickle")
    ...
    def test_topic(self, connection):
        ...
        with connection as conn, conn.channel() as channel:
            self._publish(channel, ex, [test_queue1, test_queue2, test_queue3], routing_key="t.1")
            self._consume(conn, test_queue1)
            self._consume(conn, test_queue2)
            with pytest.raises(socket.timeout):
    ...
    def test_publish_empty_exchange(self, connection):
        ...
        with connection as conn, conn.channel() as channel:
            self._publish(channel, ex, routing_key="t.1")
...
class BaseTimeToLive:
    ...
    def test_simple_queue_publish_consume(self, connection):
        with connection as conn, closing(conn.SimpleQueue("ttl_simple_queue_test")) as queue:
            queue.put({"Hello": "World"}, headers={"k1": "v1"}, expiration=2)
            sleep(3)
            with pytest.raises(queue.Empty):
                queue.get(timeout=1)

    def test_simple_buffer_publish_consume(self, connection):
        with connection as conn, closing(conn.SimpleBuffer("ttl_simple_buffer_test")) as buf:
...
class BasePriority:
    ...
    def test_publish_consume(self, connection):
        ...
        test_queue = kombu.Queue("priority_test", routing_key="priority_test", max_priority=10)
    ...
    def test_publish_requeue_consume(self, connection):
        ...
        test_queue = kombu.Queue("priority_requeue_test", routing_key="priority_requeue_test", max_priority=10)
    ...
    def test_simple_queue_publish_consume(self, connection):
        ...
        with (
            connection as conn,
            closing(conn.SimpleQueue("priority_simple_queue_test", queue_opts={"max_priority": 10})) as queue,
        ):
            for msg, prio in [
                [{"msg": "first"}, prio_low],
                [{"msg": "second"}, prio_high],
                ...
                queue.put(msg, headers={"k1": "v1"}, priority=prio)
            # Sleep to make sure that queue sorted based on priority
            sleep(0.5)
            # Second message must be received first
            for data in [
    ...
    def test_simple_buffer_publish_consume(self, connection):
        ...
        with (
            connection as conn,
            closing(conn.SimpleBuffer("priority_simple_buffer_test", queue_opts={"max_priority": 10})) as buf,
        ):
            for msg, prio in [
                [{"msg": "first"}, prio_low],
                [{"msg": "second"}, prio_high],
                ...
                buf.put(msg, headers={"k1": "v1"}, priority=prio)
            # Sleep to make sure that queue sorted based on priority
            sleep(0.5)
            # Second message must be received first
            for data in [
...
class BaseMessage:
    def test_ack(self, connection):
        with connection as conn, closing(conn.SimpleQueue("test_ack")) as queue:
            queue.put({"Hello": "World"}, headers={"k1": "v1"})
    ...
    def test_reject_no_requeue(self, connection):
        with connection as conn, closing(conn.SimpleQueue("test_reject_no_requeue")) as queue:
            queue.put({"Hello": "World"}, headers={"k1": "v1"})
            message = queue.get_nowait()
    ...
    def test_reject_requeue(self, connection):
        with connection as conn, closing(conn.SimpleQueue("test_reject_requeue")) as queue:
            queue.put({"Hello": "World"}, headers={"k1": "v1"})
            message = queue.get_nowait()
    ...
    def test_requeue(self, connection):
        with connection as conn, closing(conn.SimpleQueue("test_requeue")) as queue:
            queue.put({"Hello": "World"}, headers={"k1": "v1"})
            message = queue.get_nowait()
            message.requeue()
            message2 = queue.get_nowait()
...
class BaseFailover(BasicFunctionality):
    ...
    def test_simple_buffer_publish_consume(self, failover_connection):
        super().test_simple_buffer_publish_consume(failover_connection)
