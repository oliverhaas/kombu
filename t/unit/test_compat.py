...
class test_misc:
    ...
    def test_Queue_from_dict(self):
        ...
        assert Queue.from_dict("foo", **dict(defs)) == Queue.from_dict("foo", **dict(defs))
...
class test_Publisher:
    ...
    def test_constructor(self):
        pub = compat.Publisher(self.connection, exchange="test_Publisher_constructor", routing_key="rkey")
        ...
        explicit = Exchange("test_Publisher_constructor_explicit", type="topic")
    ...
    def test_send(self):
        pub = compat.Publisher(self.connection, exchange="test_Publisher_send", routing_key="rkey")
        pub.send({"foo": "bar"})
...
class test_Consumer:
    ...
    def test_constructor(self, n="test_Consumer_constructor"):
        c = compat.Consumer(self.connection, queue=n, exchange=n, routing_key="rkey")
...
class test_ConsumerSet:
    ...
    def test_constructor(self, prefix="0daf8h21"):
        dcon = {
            "%s.xyx" % prefix: {"exchange": "%s.xyx" % prefix, "routing_key": "xyx"},
            "%s.xyz" % prefix: {"exchange": "%s.xyz" % prefix, "routing_key": "xyz"},
        }
        consumers = [
            compat.Consumer(self.connection, queue=prefix + str(i), exchange=prefix + str(i)) for i in range(3)
        ]
        ...
        c.add_consumer(compat.Consumer(self.connection, queue=prefix + "xaxxxa", exchange=prefix + "xaxxxa"))
