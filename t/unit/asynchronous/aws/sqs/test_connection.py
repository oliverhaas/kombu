...
from kombu.asynchronous.aws.sqs.connection import AsyncSQSConnection, _query_object_encode
...
class test_AsyncSQSConnection(AWSCase):
    def setup_method(self):
        ...
        self.sqs_client.get_queue_url = MagicMock(return_value={"QueueUrl": "http://aws.com"})
    ...
    def MockServiceModel(self, operation_name, method):
        ...
        service_model.operation_model = MagicMock(return_value=self.MockOperationModel(operation_name, method))
    ...
    def test_create_query_request_get(self):
        ...
        self.assert_requests_equal(
            req,
            AWSRequest(
                url=SQS_URL,
                method=verb,
                data=None,
                params={"Action": operation_name, **params},
                headers={},
            ),
        )
    ...
    def test_create_query_request(self):
        ...
        verb = "POST"
        req = self.x._create_query_request(operation_name, params, queue_url, verb)
        self.assert_requests_equal(
            req,
            AWSRequest(
                url=queue_url,
                method=verb,
                data={"Action": operation_name, **params},
                headers={
    ...
    def test_create_json_request(self):
        ...
        self.assert_requests_equal(
            ...
            AWSRequest(
                url=SQS_URL,
                method=method,
                data=json.dumps({**params, "QueueUrl": queue_url}).encode(),
                headers={"Content-Type": "application/x-amz-json-1.0", "X-Amz-Target": f"sqs.{operation_name}"},
            ),
        )
    ...
    def test_make_request__with_query_protocol(self):
        ...
        params = {"MaxNumberOfMessages": 10, "WaitTimeSeconds": 20}
        pparams = {
        ...
        self.x._create_query_request.assert_called_with(operation, expect_params, queue_url, verb)
    ...
    def test_make_request__with_json_protocol(self):
        ...
        self.x.make_request(operation, params, queue_url, verb, protocol_params=pparams)
        self.x._create_json_request.assert_called_with(operation, expect_params, queue_url)
    ...
    def test_create_queue__with_visibility_timeout(self):
        ...
        self.x.get_object.assert_called_with(
            "CreateQueue",
            {"QueueName": "foo", "DefaultVisibilityTimeout": "33"},
            callback=self.callback,
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                None,
                ["SQSExtraAttr"],
                {"MessageSystemAttributeName.1": "ApproximateReceiveCount", "MessageAttributeNames.1": "SQSExtraAttr"},
                {"MessageSystemAttributeNames": ["ApproximateReceiveCount"], "MessageAttributeNames": ["SQSExtraAttr"]},
            ),
        ],
    )
    def test_receive_message__with_fetch_attributes(
        self,
        msg_system_attribute_names,
        msg_attribute_names,
        expected_query,
        expected_json,
    ):
    ...
    def test_send_message__with_delay_seconds(self):
        ...
        self.x.get_object.assert_called_with(
            "SendMessage",
            {"MessageBody": "hello", "DelaySeconds": 303},
            queue.id,
            verb="POST",
            callback=self.callback,
        )
    ...
    def test_send_message_batch(self):
        queue = Mock(name="queue")
        messages = [self.MockMessage("1", "r1", "A"), self.MockMessage("2", "r2", "B")]
        self.x.send_message_batch(queue, [(m.id, m.get_body(), 303) for m in messages], callback=self.callback)
        self.x.get_object.assert_called_with(
            "SendMessageBatch",
            {
