...
from pymongo.collection import Collection  # isort:skip
from pymongo.database import Database  # isort:skip
from kombu.transport.mongodb import BroadcastCursor  # isort:skip
...
class test_mongodb_uri_parsing:
    ...
    def test_flattens_single_item_and_lowercases(self, monkeypatch):
        ...
        fake = {"database": "test", "options": {"replicaSet": ["rs0"], "readPreference": ["primary"]}}
    ...
    def test_multi_item_lists_not_flattened(self, monkeypatch):
        ...
        fake = {"database": "test", "options": {"readPreferenceTags": ["dc:east", "use:analytics"]}}
        monkeypatch.setattr(mod.uri_parser, "parse_uri", lambda *a, **k: fake)
    ...
    def test_equal_values_different_casing_no_warning(self, monkeypatch, recwarn):
        ...
        fake = {"database": "test", "options": {"replicaSet": ["rs0"], "replicaset": ["rs0"]}}
        monkeypatch.setattr(mod.uri_parser, "parse_uri", lambda *a, **k: fake)
    ...
    def test_conflicting_values_same_key_diff_case_last_wins(self, monkeypatch, recwarn):
        ...
        fake = {"database": "test", "options": {"replicaSet": ["rs0"], "replicaset": ["DIFFERENT"]}}
        monkeypatch.setattr(mod.uri_parser, "parse_uri", lambda *a, **k: fake)
...
class BaseMongoDBChannelCase:
    ...
    def assert_collection_accessed(self, *collections):
        self.channel.client.__getitem__.assert_has_calls([call(c) for c in collections], any_order=True)
...
class test_mongodb_channel(BaseMongoDBChannelCase):
    ...
    def test_get_fanout(self):
        self.set_broadcast_return_value(
            "foobar",
            {
                "_id": "docId1",
                "payload": '{"some": "data"}',
    ...
    def test_queue_bind(self):
        ...
        self.assert_operation_called_with(
            "routing",
            "update_one",
            {"queue": "foo", "pattern": "*", "routing_key": "foo", "exchange": "test_exchange"},
            {"$set": {"queue": "foo", "pattern": "*", "routing_key": "foo", "exchange": "test_exchange"}},
            upsert=True,
        )
...
class test_mongodb_channel_ttl(BaseMongoDBChannelCase):
    def setup_method(self):
        ...
        self.expire_at = self.channel.get_now() + timedelta(milliseconds=777)
    ...
    def test_queue_delete(self):
        ...
        self.assert_operation_called_with("queues", "delete_one", {"_id": "foobar"})

    def test_ensure_indexes(self):
        self.channel._ensure_indexes(self.channel.client)

        self.assert_operation_called_with("messages", "create_index", [("expire_at", 1)], expireAfterSeconds=0)

        self.assert_operation_called_with("routing", "create_index", [("expire_at", 1)], expireAfterSeconds=0)

        self.assert_operation_called_with("queues", "create_index", [("expire_at", 1)], expireAfterSeconds=0)

    def test_get_queue_expire(self):
        result = self.channel._get_queue_expire({"arguments": {"x-expires": 777}}, "x-expires")
    ...
    def test_get_message_expire(self):
        assert (
            self.channel._get_message_expire(
                {
