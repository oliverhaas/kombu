...
class SQSClientMock:
    ...
    def list_queues(self, QueueNamePrefix=None):
        ...
        urls = (val.url for key, val in self._queues.items() if key.startswith(QueueNamePrefix))
    ...
    def send_message(self, QueueUrl=None, MessageBody=None, MessageAttributes=None):
        for q in self._queues.values():
            if q.url == QueueUrl:
                handle = "".join(random.choice(string.ascii_lowercase) for x in range(10))
                q.messages.append(
                    {"Body": MessageBody, "ReceiptHandle": handle, "MessageAttributes": MessageAttributes},
                )
                break
    ...
    def receive_message(
        ...
        for q in self._queues.values():
            if q.url == QueueUrl:
                ...
                for msg in msgs:
                    msg["Attributes"] = {"SenderId": "AIDAEXAMPLE123ABC", "SentTimestamp": "1638368280000"}
...
class test_Channel:
    ...
    def setup_method(self):
        ...
        self.producer = messaging.Producer(self.channel, self.exchange, routing_key=self.queue_name)
    ...
    def test_region(self):
        ...
        connection = Connection(transport=SQS.Transport, transport_options={"region": "us-west-2"})
        channel = connection.channel()
    ...
    def test_resolve_queue_url(self):
        ...
        assert self.sqs_conn_mock._queues[queue_name].url == self.channel._resolve_queue_url(queue_name)
    ...
    def test_dont_create_duplicate_new_queue(self):
        ...
        assert queue.messages[0]["Body"] == "hello"
    ...
    def test_delete_message(self, queue_name, message, new_q_url, monkeypatch):
        ...
        fake_sqs.delete_message.assert_called_once_with(new_q_url, message["ReceiptHandle"])
    ...
    def test_messages_to_python(self):
        ...
        kombu_messages = []
        for m in self.sqs_conn_mock.receive_message(QueueUrl=q_url, MaxNumberOfMessages=kombu_message_count)[
            "Messages"
        ]:
    ...
    @pytest.mark.parametrize("body", ["", "True", "False", "[]", "{}", "1", "1.0", "This is the body as a string"])
    def test_messages_to_python_body_as_a_string(self, body):
        ...
        self.sqs_conn_mock.send_message(q_url, MessageBody=body, MessageAttributes=msg_attributes)

        received_messages = []
        for m in self.sqs_conn_mock.receive_message(QueueUrl=q_url, MaxNumberOfMessages=1)["Messages"]:
            m["Body"] = Message(body=m["Body"]).decode()
            received_messages.append(m)

        formatted_messages = self.channel._messages_to_python(received_messages, self.queue_name)
    ...
    def test_redelivered(self):
        ...
        message = {"redelivered": True, "properties": {"delivery_tag": "test_message_id"}}
        self.channel._put(self.producer.routing_key, message)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (None, {"message_system_attribute_names": ["ApproximateReceiveCount"], "message_attribute_names": []}),
            (
                "incorrect_value",
                {"message_system_attribute_names": ["ApproximateReceiveCount"], "message_attribute_names": []},
            ),
            ([], {"message_system_attribute_names": ["ApproximateReceiveCount"], "message_attribute_names": []}),
            (["ALL"], {"message_system_attribute_names": ["ALL"], "message_attribute_names": []}),
            (
            ...
            (
                ...
                {"message_system_attribute_names": ["ApproximateReceiveCount"], "message_attribute_names": ["ALL"]},
            ),
            (
    ...
    def test_basic_ack_invalid_receipt_handle(self, basic_reject_mock, basic_ack_mock):
        ...
        error_response = {
            "Error": {
                ...
                "Message": "Value 2 for parameter ReceiptHandle is invalid. Reason: The receipt handle has expired.",
            },
        }
    ...
    def test_sts_new_session(self):
        ...
        connection = Connection(
            transport=SQS.Transport,
            transport_options={"predefined_queues": example_predefined_queues, "sts_role_arn": "test::arn"},
        )
        channel = connection.channel()
        ...
        mock_generate_sts_session_token.side_effect = [
            {"Expiration": 123, "SessionToken": 123, "AccessKeyId": 123, "SecretAccessKey": 123},
        ]
    ...
    def test_sts_new_session_with_buffer_time(self):
        ...
        mock_generate_sts_session_token.side_effect = [
            {"Expiration": expiration_time, "SessionToken": 123, "AccessKeyId": 123, "SecretAccessKey": 123},
        ]
        channel.generate_sts_session_token = mock_generate_sts_session_token
    ...
    def test_message_attribute(self):
        ...
        self.producer.publish(
            message,
            message_attributes={"Attribute1": {"DataType": "String", "StringValue": "STRING_VALUE"}},
        )
