...
class test_Direct(ExchangeCase):
    ...
    table = [("rFoo", None, "qFoo"), ("rFoo", None, "qFox"), ("rBar", None, "qBar"), ("rBaz", None, "qBaz")]

    @pytest.mark.parametrize(
        "exchange,routing_key,default,expected",
        [
    ...
    def test_lookup(self, exchange, routing_key, default, expected):
        assert self.e.lookup(self.table, exchange, routing_key, default) == expected


class test_Fanout(ExchangeCase):
    type = exchange.FanoutExchange
    table = [(None, None, "qFoo"), (None, None, "qFox"), (None, None, "qBar")]
...
class test_Topic(ExchangeCase):
    ...
    def setup_method(self):
        super().setup_method()
        self.table = [(rkey, self.e.key_to_pattern(rkey), queue) for rkey, _, queue in self.table]
...
class test_ExchangeType(ExchangeCase):
    ...
    @pytest.mark.parametrize(
        "ex,eq,name,type,durable,auto_delete,arguments",
        [
    ...
    def test_equivalent(self, ex, eq, name, type, durable, auto_delete, arguments):
        is_eq = self.e.equivalent(ex, name, type, durable, auto_delete, arguments)
