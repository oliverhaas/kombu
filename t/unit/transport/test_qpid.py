...
class test_Connection__init__(ConnectionTestBase):
    ...
    @patch(QPID_MODULE + ".sys.exc_info")
    @patch(QPID_MODULE + ".qpid")
    def test_connection__init__mutates_ConnError_by_message2(self, mock_qpid, mock_exc_info):
        """
...
class test_Channel_put(ChannelTestBase):
    ...
    def test_channel__put_onto_queue(self, mock_qpid):
        ...
        address_str = f"{routing_key}; {{assert: always, node: {{type: queue}}}}"
        self.transport.session.sender.assert_called_with(address_str)
        mock_Message_cls.assert_called_with(content=mock_message, subject=None, durable=True)
        mock_sender = self.transport.session.sender.return_value
        mock_sender.send.assert_called_with(
            mock_Message_cls.return_value,
            sync=True,
        )
    ...
    def test_channel__put_onto_exchange(self, mock_qpid):
        ...
        addrstr = f"{mock_exchange_name}/{mock_routing_key}; {{assert: always, node: {{type: topic}}}}"
        self.transport.session.sender.assert_called_with(addrstr)
        mock_Message_cls.assert_called_with(content=mock_message, subject=mock_routing_key, durable=True)
        mock_sender = self.transport.session.sender.return_value
        mock_sender.send.assert_called_with(
            mock_Message_cls.return_value,
            sync=True,
...
class test_Channel:
    ...
    def test_queue_declare_with_exception_raised(self, mock_queue_declare_ok_t):
    ...
    def test_prepare_message(self):
        ...
        properties = {"properties1": mock_properties1, "properties2": mock_properties2}
        ...
        assert mock_priority is result["properties"]["delivery_info"]["priority"]
    ...
    def test_basic_publish(self, mock_put, mock_encode_body, mock_body_encoding, mock_buffer):
        """Test basic_publish()."""
        mock_original_body = Mock()
        mock_encoded_body = "this is my encoded body"
        mock_message = {"body": mock_original_body, "properties": {"delivery_info": {}}}
        ...
        assert mock_message["properties"]["body_encoding"] is mock_body_encoding
        assert isinstance(mock_message["properties"]["delivery_tag"], uuid.UUID)
        assert mock_message["properties"]["delivery_info"]["exchange"] is mock_exchange
        assert mock_message["properties"]["delivery_info"]["routing_key"] is mock_routing_key
...
class test_Transport__init__:
    ...
    def mock_verify_runtime_environment(self, patching):
        self.mock_verify_runtime_environment = patching.object(Transport, "verify_runtime_environment")

    @pytest.fixture(autouse=True)
    def mock_transport_init(self, patching):
        self.mock_base_Transport__init__ = patching(QPID_MODULE + ".base.Transport.__init__")
...
class test_Transport_class_attributes:
    ...
    def test_transport_verify_pre_kombu_3_0_exception_labels(self):
        assert Transport.recoverable_channel_errors == Transport.channel_errors
        assert Transport.recoverable_connection_errors == Transport.connection_errors
...
class test_Transport_on_readable:
    ...
    def test_transport_on_readable_catches_socket_timeout(self):
        self.mock_drain_events.side_effect = TimeoutError()
        self.transport.on_readable(Mock(), Mock())

    def test_transport_on_readable_ignores_non_socket_timeout_exception(self):
        self.mock_drain_events.side_effect = OSError()
        with pytest.raises(IOError):
            self.transport.on_readable(Mock(), Mock())
