...
DEFAULT_AZURE_URL_CREDS = (
    "azurestoragequeues://DefaultAzureCredential@https://STORAGE_ACCOUNT_NAME.queue.core.windows.net/"
)
MANAGED_IDENTITY_URL_CREDS = (
    "azurestoragequeues://ManagedIdentityCredential@https://STORAGE_ACCOUNT_NAME.queue.core.windows.net/"
)


def test_queue_service_nocredentials():
    conn = Connection(URL_NOCREDS, transport=azurestoragequeues.Transport)
    with pytest.raises(ValueError, match="Need a URI like azurestoragequeues://{SAS or access key}@{URL}"):
        conn.channel()
...
def test_queue_service_works_for_default_azure_credentials():
    conn = Connection(DEFAULT_AZURE_URL_CREDS, transport=azurestoragequeues.Transport)
    with patch("kombu.transport.azurestoragequeues.QueueServiceClient"):
        channel = conn.channel()
...
def test_queue_service_works_for_managed_identity_credentials():
    conn = Connection(MANAGED_IDENTITY_URL_CREDS, transport=azurestoragequeues.Transport)
    with patch("kombu.transport.azurestoragequeues.QueueServiceClient"):
        channel = conn.channel()
