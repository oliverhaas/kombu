        ...
        class Receiver:
            ...
            def receive_messages(_self, **kwargs2):
                ...
                self.recv_calls.append(
                    {"receiver_options": kwargs, "receive_messages_options": kwargs2, "messages": result},
                )
                return result
...
def test_custom_wait_timeout_seconds():
    conn = Connection(URL_CREDS_SAS, transport=azureservicebus.Transport, transport_options={"wait_time_seconds": 10})
    channel = conn.channel()
...
def test_custom_peek_lock_seconds():
    conn = Connection(URL_CREDS_SAS, transport=azureservicebus.Transport, transport_options={"peek_lock_seconds": 65})
    channel = conn.channel()
...
def test_invalid_peek_lock_seconds():
    # Max is 300
    conn = Connection(URL_CREDS_SAS, transport=azureservicebus.Transport, transport_options={"peek_lock_seconds": 900})
    channel = conn.channel()
...
def mock_queue(mock_asb, mock_asb_management, random_queue) -> MockQueue:
    ...
    return MockQueue(random_queue, mock_asb, mock_asb_management, conn, channel, producer, queue)
...
def test_basic_ack_complete_message(mock_queue: MockQueue):
    ...
    message = mock_queue.channel._get(mock_queue.queue_name)
    mock_queue.channel.qos.get = MagicMock(return_value=mock_queue.channel.Message(message, mock_queue.channel))
...
def test_basic_ack_when_already_settled(mock_queue: MockQueue):
    ...
    mock_queue.channel.qos.get = MagicMock(return_value=mock_queue.channel.Message(message, mock_queue.channel))
    receiver_mock = MagicMock()
    receiver_mock.complete_message = MagicMock(side_effect=azure.servicebus.exceptions.MessageAlreadySettled())
    queue_object_mock = MagicMock()
    queue_object_mock.receiver = receiver_mock
    mock_queue.channel._get_asb_receiver = MagicMock(return_value=queue_object_mock)
    with patch("kombu.transport.virtual.base.Channel.basic_ack") as super_basic_ack:
