...
class Producer:
    ...
    async def apublish(
        self, body, routing_key=None, delivery_mode=None,
        mandatory=False, immediate=False, priority=0,
        content_type=None, content_encoding=None, serializer=None,
        headers=None, compression=None, exchange=None, retry=False,
        retry_policy=None, declare=None, expiration=None, timeout=None,
        confirm_timeout=None, **properties
    ):
        """Async publish message to the specified exchange.

        This is the async version of :meth:`publish`.
        ...
        Arguments:
            ...
            expiration (float): A TTL in seconds can be specified per message.
            timeout (float): Set timeout for publish.
            confirm_timeout (float): Set confirm timeout for publish confirmation.
            **properties (Any): Additional message properties.
        """
        declare = [] if declare is None else declare
        headers = {} if headers is None else headers
        retry_policy = {} if retry_policy is None else retry_policy
    ...
    async def _apublish(
        ...
        if declare:
            for entity in declare:
                if hasattr(entity, 'adeclare'):
                    ...
                    # Fallback to sync declare if async not available
                    maybe_declare(entity, channel, retry=retry, **retry_policy)
        ...
        # Use async basic_publish if available
        if hasattr(channel, 'abasic_publish'):
            return await channel.abasic_publish(
                message,
                exchange=exchange, routing_key=routing_key,
                mandatory=mandatory, immediate=immediate,
            )
        else:
            # Fallback to sync if channel doesn't support async
            return channel.basic_publish(
                message,
                exchange=exchange, routing_key=routing_key,
                mandatory=mandatory, immediate=immediate,
...
class Consumer:
    ...
    async def aconsume(self, no_ack=None):
        """Async start consuming messages from queues.

        This is the async version of :meth:`consume`.
        ...
        for queue in queues:
            tag = await self._abasic_consume(queue, no_ack=no_ack)
        self._active_tags[queue.name] = tag
        return [tag]

    async def _abasic_consume(
        self, queue, consumer_tag=None,
        no_ack=no_ack, nowait=True
    ):
        """Async basic consume from queue."""
        tag = self._active_tags.get(queue.name)
        if tag is None:
            tag = self._add_tag(queue, consumer_tag)
            # For async, we still use sync basic_consume as
            # the actual consumption happens via drain_events
            queue.consume(
                tag, self._receive_callback,
                no_ack=no_ack, nowait=nowait
            )
        return tag

    async def acancel(self):
        """Async cancel all consumers.

        This is the async version of :meth:`cancel`.
        """
        cancel = self.channel.basic_cancel
        for tag in self._active_tags.values():
            cancel(tag)
