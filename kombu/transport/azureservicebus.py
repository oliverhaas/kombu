...
class Channel(virtual.Channel):
    ...
    def _get(self, queue: str, timeout: float | None = None) -> dict[str, Any]:
        ...
        recv_mode = (
            ServiceBusReceiveMode.RECEIVE_AND_DELETE if queue in self._noack_queues else ServiceBusReceiveMode.PEEK_LOCK
        )
    ...
    def parse_uri(uri: str) -> tuple[str, str | DefaultAzureCredential | ManagedIdentityCredential]:
        ...
        if "DefaultAzureCredential".lower() == credential.lower():
            if DefaultAzureCredential is None:
                raise ImportError(
                    "Azure Service Bus transport with a DefaultAzureCredential requires the azure-identity library"
                )
            credential = DefaultAzureCredential()
        elif "ManagedIdentityCredential".lower() == credential.lower():
            if ManagedIdentityCredential is None:
                raise ImportError(
                    "Azure Service Bus transport with a ManagedIdentityCredential requires the azure-identity library"
                )
