...
class Channel(virtual.Channel):
    ...
    def _new_queue(self, queue, **kwargs):
        ...
        try:
            ...
            try:
                self.slmq.create_queue(queue, visibility_timeout=self.visibility_timeout)
    ...
    def _get(self, queue):
        ...
        if rs["items"]:
            ...
            if queue in self._noack_queues:
                ...
                payload["properties"]["delivery_info"].update({"slmq_message_id": m["id"], "slmq_queue_name": q.name})
            return payload
        raise Empty()
    ...
    def slmq(self):
        if self._slmq is None:
            ...
            secure = bool(
                os.environ.get("SLMQ_SECURE", self.transport_options.get("secure")) or True,
            )
            endpoint = "{}://{}{}".format(
...
class Transport(virtual.Transport):
    ...
    polling_interval = 1
    default_port = None
    connection_errors = virtual.Transport.connection_errors + (ResponseError, socket.error)
